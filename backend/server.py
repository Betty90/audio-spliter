...
def setup_ffmpeg():
    ...
    if found_path:
        ...
        logging.warning("FFmpeg binary not found in expected locations. Relying on system PATH.")
...
def process_audio(file_path, n_clusters=2, min_duration=0.5, silence_thresh=0.005, hop_length=512, smoothing_width=5, sample_rate=16000, min_silence_duration=0.1):
    """
    使用 Librosa 提取 MFCC 特征，并使用 K-Means 聚类进行说话人区分。
    这是一个轻量级的机器学习方案，不需要 GPU。
    """
    try:
        logging.info(f"开始处理文件: {file_path}, 聚类数: {n_clusters}, hop_length: {hop_length}, smoothing: {smoothing_width}, sr: {sample_rate}, min_silence: {min_silence_duration}")
        ...
        try:
            ...
            try:
                ...
                if samplerate != sample_rate:
                    y = librosa.resample(y=data.T, orig_sr=samplerate, target_sr=sample_rate)
                    sr = sample_rate
                else:
                    y = data.T
                    sr = sample_rate
                ...
                raise ValueError(f"无法加载音频文件 (可能是格式不支持或 FFmpeg 未安装): {str(load_err)}")
        ...
        mfcc = librosa.feature.mfcc(y=y_pre, sr=sr, n_mfcc=n_mfcc, hop_length=hop_length)
        # 计算一阶差分 (Delta)
        ...
        if n_samples < n_clusters:
            logging.warning(f"样本数 ({n_samples}) 小于聚类数 ({n_clusters})，自动调整聚类数。")
        ...
        try:
            gmm = GaussianMixture(n_components=n_clusters, covariance_type='diag', n_init=5, random_state=42)
        ...
        logging.info(f"Min silence duration: {min_silence_duration}s -> {min_silence_frames} frames")
        ...
        logging.info(f"Silence filling: Found {silence_runs_found} runs, Filled {silence_runs_filled} runs")
            ...
            if effective_label != current_label:
                if current_label is not None:
                    ...
                    if (end_time - start_time) >= min_duration:
                        # -1 代表噪音/静音
                        speaker_name = "噪音/静音" if current_label == -1 else f"音色 {int(current_label) + 1}"
        ...
        logging.info(f"处理完成，生成 {len(segments)} 个片段. Dropped {segments_dropped_duration} segments due to min_duration.")
        ...
        if len(segments) > 0:
            ...
            for next_seg in segments[1:]:
                ...
                if current_seg['speaker'] == next_seg['speaker']:
                    ...
                    if gap < min_silence_duration:
                        ...
                        logging.info(f"Merged segments of {current_seg['speaker']} with gap {gap:.2f}s")
                        continue
            ...
            if len(segments) != len(merged_segments):
                logging.info(f"Post-processing merged {len(segments) - len(merged_segments)} gaps.")
...
def convert_audio():
    ...
    target_format = request.form.get('target_format') or request.form.get('targetFormat') or 'mp4'
    ...
    try:
        ...
        return send_file(
            temp_output.name,
            mimetype=f'audio/{target_format}' if target_format in ['mp3', 'wav', 'm4a'] else f'video/{target_format}',
            as_attachment=True,
...
def health():
    if request.method == 'OPTIONS':
        return jsonify({'status': 'ok'}), 200
    return jsonify({"status": "ok", "message": "Python audio service is running"})

if __name__ == '__main__':
    # Change port to 5001 to avoid conflicts with macOS AirPlay (port 5000)
    PORT = 5001
